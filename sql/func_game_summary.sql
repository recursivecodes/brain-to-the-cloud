...
begin
    return q'{
        select
            ...
            case when gameSummary.withBrain = 1 then cast(coalesce(avg(avgAttention), 0) as number(18,2)) else null end as avgAttention,
            case when gameSummary.withBrain = 1 then cast(coalesce(avg(avgMeditation), 0) as number(18,2)) else null end as avgMeditation,
